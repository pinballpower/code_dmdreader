...
void Pi4Renderer::recalc_vertices() {
...
void Pi4Renderer::render_frame(DMDFrame& f)
{
	...
	if (tx_buf_len < tx_pixel_count * 4) {
		BOOST_LOG_TRIVIAL(error) << "[Pi4Renderer] buffer too small, aborting";
		return;
	}
...
bool Pi4Renderer::initialize_display()
{
	...
	if (data)
	{
		...
		BOOST_LOG_TRIVIAL(debug) << "[Pi4Renderer] loaded overlay_texture " << overlay_texture_file;
		stbi_image_free(data);
	}
	else
	{
		BOOST_LOG_TRIVIAL(warning) << "[Pi4Renderer] Failed to load overlay_texture " << overlay_texture_file << ", will use no overlay";
...
bool Pi4Renderer::configure_from_ptree(boost::property_tree::ptree pt_general, boost::property_tree::ptree pt_renderer)
{
